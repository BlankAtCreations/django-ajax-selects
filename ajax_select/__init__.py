...
def make_ajax_form(model, fieldlist, superclass=ModelForm, show_help_text=False, **kwargs):
    ...
    class TheForm(superclass):
        ...
        def save(self, *args, **kwargs):
            ...
            instance = super(TheForm, self).save(*args, commit=False, **kwargs)
            if commit:
                instance.save()
            return instance

    for model_fieldname, channel in fieldlist.iteritems():
        f = make_ajax_field(model, model_fieldname, channel, show_help_text, **kwargs)
...
def make_ajax_field(model, model_fieldname, channel, show_help_text=False, **kwargs):
    ...
    if not hasattr(kwargs, 'plugin_options'):
        kwargs['plugin_options'] = {}
    kwargs['plugin_options'].update({
